module ApplicationHelper
    ...
    def friend_request_received?(user)
        current_user.friend_request.exist?(sent_by_id: user.id, status: false)
    end

    def possible_friend?(user)
        request_sent = current_user.friend_sent.exist?(sent_to_id: user.id)
        request_received = current_user.friend_request.exist?(sent_by_id: user.id)
